...
class Dungeon {
              ...
              roomnum, roomdat, forprop, stp, corridorwidth)
  {
    this.stats = {
      ...
      corridor:{
        forward:Math.max(forprop, 1-forprop), //probability of going towards end vs. away from goal (must be >0.5 or the dungeon may never finish)
  ...
  generateCorridor(from, to){
    ...
    let replaceMap = {};
    replaceMap[COLORMAP.floor] = COLORMAP.floor;
    ...
    let fillCorridorSlice = (x, y) => {
      ...
      for (let i = -1; i <= this.corridorwidth; i++){ //top down
        for (let j = -1; j <= this.corridorwidth; j++){ //left right
          ...
          this.map[p[0]][p[1]] = COLORMAP.floor;
  ...
  generateRoom(x1, y1, x2, y2){
    let rd = [x2-x1, y2-y1];
    let roomMap = zeros(rd, COLORMAP.floor);
...
function randomDungeon(seed, w, h){
  ...
  let roomMaxSize = 100; //max room width/height
  ...
  let genRandomRoom = () => {
    ...
    room.dims[0] = Math.floor(100/(1+100**(-2*(1/(1.7-rng())-1)))+corridorwidth);
    let roomRatio = rng()**0.25;
    if (rng() < 0.5){ //bigger width
      room.dims[1] = Math.floor(roomRatio*room.dims[0]);
    }else{ //bigger height
      room.dims[1] = room.dims[0];
      room.dims[0] = Math.floor(roomRatio*room.dims[1]);
    }
  ...
  let straightness = (1-Math.cos(Math.PI*rng())**8)*0.6;
